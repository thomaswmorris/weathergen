...
.. code-block:: python
    ...
    gen_time = np.arange(t0, t1, 3600) # three months of hourly data

    weather = weathergen.generate(site='princeton', seasonal=True, diurnal=True)
    weather.generate(time=gen_time, method='random')
...
Note that the supplied year is arbitrary; the underlying model considers only seasonal and diurnal climatological variations. All available sites are outlined in the dataframe ``weathergen.regions``.
